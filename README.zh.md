...
[![Tools](https://img.shields.io/badge/Tools-379-orange.svg)](#-工具分类)
[![Bybit V5 API](https://img.shields.io/badge/Bybit-V5%20API-green.svg)](https://bybit-exchange.github.io/docs/v5/intro)

**生产就绪的 Bybit MCP 服务器 — 379 个工具，覆盖行情数据、交易下单、仓位管理、账户管理、资产查询、借贷、理财和 WebSocket 实时推送**
...
- **完整的 V5 覆盖** — 379 个工具，涵盖行情、交易、仓位、账户、资产、借贷、理财、跟单、WebSocket 和 WS 交易等类别
...
| `account` | 是 | 24 | 钱包余额、交易日志、手续费率、保证金模式、抵押品开关（单个/批量）、对冲模式、限价操作、MMP 修改与重置、期权希腊值、断线保护、防自成交分组、账户合约、可提金额，以及 UTA 借还款与借款历史 |
...
| `user` | 是 | 8 | 只读的用户与子账户信息：API Key 信息与权限、子账户及托管子账户列表、成员账户类型、邀请码与被邀请用户查询 |
| `affiliate` | 是 | 2 | 联盟返佣：返佣用户列表、单个用户返佣信息 |
| `broker` | 是 | 8 | 经纪商：收益与账户信息、奖励信息/发放/记录、API 限速设置与查询（上限及按 UID） |
...
| `websocket` | 混合 | 27 | 订阅-快照模式实时数据：订单薄、Ticker、K 线、成交、强平、订单、仓位、钱包、期权希腊值、RFQ 大宗交易、价差合约等 |
